...
obj.colorList=1;
obj.pieceNum=6;
obj.varName={'NRE','Nup','BNF'}; %这里的标签和顺序要注意核对
obj.title='';
...
% =========================================================================
% 基础配色表
baseAtla{1}=[1,0,0;0,1,0;0,0,1]; %红绿蓝
baseAtla{2}=[0,211,166;211,152,255;233,164,67]./255;
baseAtla{3}=[255,0,255;255,255,0;0,255,255]./255;
...
% CMapData=getCMap3(A,B,C,atla,obj.pieceNum);
    ...
    function CMap=getCMap3(A,B,C,atla,n)
        ...
        ABC=[nA,nB,nC];ABC=ABC./sum(ABC,2);ABC(isnan(ABC))= 1;
...
text(ax2,cos(pi/2+2*pi/3).*0.65,sin(pi/2+2*pi/3).*0.8,obj.varName{1},'FontSize',18,'FontName','Times','HorizontalAlignment','center')
text(ax2,cos(pi/2).*0.69,sin(pi/2).*0.69,obj.varName{2},'FontSize',18,'FontName','Times','HorizontalAlignment','center')
text(ax2,cos(pi/2-2*pi/3).*0.68,sin(pi/2-2*pi/3).*0.8,obj.varName{3},'FontSize',18,'FontName','Times','HorizontalAlignment','center')

% 绘制箭头方向
% text(ax2,cos(-pi/2).*0.4,sin(-pi/2).*0.4,'→','FontSize',15,'FontName','Times','HorizontalAlignment','center')
% text(ax2,cos(-pi/2+2*pi/3).*0.4,sin(-pi/2+2*pi/3).*0.4,'→','FontSize',15,'FontName','Times','HorizontalAlignment','center','Rotation',120)
% text(ax2,cos(-pi/2-2*pi/3).*0.4,sin(-pi/2-2*pi/3).*0.4,'→','FontSize',15,'FontName','Times','HorizontalAlignment','center','Rotation',-120)
